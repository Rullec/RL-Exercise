...
class PPOAgent(object):
    ...
    def get_action(self, s):
        ...
        a = self.sess.run(self.sample_op, {self.s: s})
        return a
    ...
    def compute_target_value(self, reward_lst, final_state):
        ...
        v_final = self.sess.run(self.value, feed_dict = {self.s : final_state})
        cur_value = v_final
        target_lst = []
    ...
    def train_one_step(self):
        ...
        while step < self.max_step and not done:
            ...
            bootstrap_value = self.get_v(next_state)
    ...
    def train(self, s, a, r):
        ...
        adv = self.sess.run(self.adv, {self.s: s, self.y: r})
        feed_dict_actor = {}
        feed_dict_actor[self.s] = s
        feed_dict_actor[self.a] = a
        feed_dict_actor[self.advantage] = adv
        feed_dict_critic = {}
        feed_dict_critic[self.s] = s
        feed_dict_critic[self.y] = r
