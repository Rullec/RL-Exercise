...
with tf.Session() as sess:
    ...
    episode_score = 0
    episode_steps = 0
    noise = UONoise()   # 创建一个噪声
    memory = Memory(memory_size)    # memory = 10000
    while episode < max_episode:
        ...
        # 对action增加噪声
        # 前100个回合总是在action上面增加噪声, 目的在于增加探索?而不是随机...
        # 换了以后，我发现人家的方法还是能收敛，探索不是最关键的地方s
        if episode<max_explore_eps: # exploration policy
            # p = episode/max_explore_eps
            # action = action*p + (1-p)*next(noise)
            if np.random.rand() > (episode * 1.0/ max_explore_eps):#可能性逐渐变小
                action = np.array([np.random.rand() * 2 - 1])
        action *= 2
        next_obs, reward, done,info = env.step(action)
        memory.append([obs,action,reward,next_obs,done])

        # 如果iter > 768，也就是大约4个episode以后，才会执行这个
